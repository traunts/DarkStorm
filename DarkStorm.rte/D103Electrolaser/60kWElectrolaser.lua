function Create(self)
	...
	local dist = SceneMan:CastObstacleRay(self.Pos, checkVect, strikePoint, lastPoint, baseUserID, ignoreTeamNumber, -1, 0);
	...
	if dist >= 0 then
		...
		if targetID ~= 255 then
			...
			for i=target.Pos.X - self.shockWidth, target.Pos.X + self.shockWidth do
				...
				if SceneMan:CastFindMORay(shockPoint, Vector(0, -self.shockHeight), target.ID, foundVector, -1, true, 0) then
					--target is grounded
					discharge = true;
					
					if target:IsActor() and target.PresetName ~= "D2-E Tengu Bushi" and target.PresetName ~= "D4-E Oni Bushi" and target.PresetName ~= "D45Shishi" and target.PresetName ~= "D45Shishi Constructor Top" and target.PresetName ~= "D45Shishi Constructor Segment" and target.PresetName ~= "D45Shishi Constructor" then
