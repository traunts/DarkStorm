function Create(self)
	...
	if MovableMan:ValidMO(user) and user:IsActor() then
		...
		if not(MovableMan:ValidMO(disruption)) then
			...
			local reload = nil;
			for particle in MovableMan.Particles do
				if particle.PresetName == "D56 Missile Launcher Reload" then
					local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
					local dist = distVect.Magnitude;
					if dist <= self.userRange then
						reload = particle;
			...
			if MovableMan:ValidMO(reload) then
				...
				if UInputMan:KeyPressed(secondaryFireKeyNum) then
					local empty = CreateAEmitter("D56 Missile Launcher Empty Sound Emitter");
					empty.Pos = self.Pos;
				...
				if reload.Mass > self.reloadTime then
					...
					local loading = CreateAEmitter("D56 Missile Launcher Load Sound Emitter");
					...
					for i = 0, MovableMan:GetMOIDCount() do
						if MovableMan:GetRootMOID(i) == user.ID then
							...
							if object.PresetName == "D56 Missile Launcher Indicator Green" then
								object.Scale = 1;
							elseif object.PresetName == "D56 Missile Launcher Indicator Blue" or object.PresetName == "D56 Missile Launcher Indicator Red" then
								object.Scale = 0;
							end
						end
					end
				...
				if userCont:IsPlayerControlled(-1) then
					...
					if UInputMan:KeyPressed(secondaryFireKeyNum) then
						...
						local fired = nil;
						for particle in MovableMan.Particles do
							if particle.PresetName == "D56 Missile Launcher Fired" then
								local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
								local dist = distVect.Magnitude;
								if dist <= self.userRange then
									fired = particle;
						...
						if MovableMan:ValidMO(fired) then
							...
							local shot = CreateAEmitter("D56 Missile Launcher Unload Sound Emitter");
							shot.Pos = self.Pos;
							MovableMan:AddParticle(shot);
							
							reload = CreateMOPixel("D56 Missile Launcher Reload");
							reload.Pos = user.Pos;
							...
							for i = 0, MovableMan:GetMOIDCount() do
								if MovableMan:GetRootMOID(i) == user.ID then
									local object = MovableMan:GetMOFromID(i);
									if object.PresetName == "D56 Missile Launcher Indicator Blue" then
										object.Scale = 1;
									elseif object.PresetName == "D56 Missile Launcher Indicator Green" or object.PresetName == "D56 Missile Launcher Indicator Red" then
										object.Scale = 0;
									end
								end
							end
							...
							--fire a shot
							local grenade = CreateAEmitter("80mm Guided Missile");
							...
							local flash = CreateMOSRotating("Muzzle Flash 40mm Underslung Missile Launcher");
							...
							for i = 0, MovableMan:GetMOIDCount() do
								if MovableMan:GetRootMOID(i) == user.ID then
									local object = MovableMan:GetMOFromID(i);
									if object.PresetName == "D56 Missile Launcher Indicator Red" then
										object.Scale = 1;
									elseif object.PresetName == "D56 Missile Launcher Indicator Green" or object.PresetName == "D56 Missile Launcher Indicator Blue" then
										object.Scale = 0;
									end
								end
							end
