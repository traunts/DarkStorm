function Create(self)
	...
	if MovableMan:ValidMO(user) and user:IsActor() then
		...
		if not(MovableMan:ValidMO(disruption)) then
			...
			local reload = nil;
			for particle in MovableMan.Particles do
				if particle.PresetName == "D37-S Pistol Reload" then
					local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
					local dist = distVect.Magnitude;
					if dist <= self.userRange then
						reload = particle;
			...
			if MovableMan:ValidMO(reload) then
				...
				if UInputMan:KeyPressed(secondaryFireKeyNum) then
					local empty = CreateAEmitter("D37-S Pistol Empty Sound Emitter");
					empty.Pos = self.Pos;
				...
				if reload.Mass > self.reloadTime then
					...
					local loading = CreateAEmitter("D37-S Pistol Load Sound Emitter");
				...
				if userCont:IsPlayerControlled(-1) then
					...
					local refire = nil;
					for particle in MovableMan.Particles do
						if particle.PresetName == "D37-S Pistol Refire" then
							local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
							local dist = distVect.Magnitude;
							if dist <= self.userRange then
								refire = particle;
					...
					if MovableMan:ValidMO(refire) then
						...
						if UInputMan:KeyPressed(secondaryFireKeyNum) and not(userCont:IsState(Controller.WEAPON_FIRE)) then
							--trigger pulled
							local noFired = 0;
							for particle in MovableMan.Particles do
								if particle.PresetName == "D37-S Pistol Fired" then
									local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
									local dist = distVect.Magnitude;
									if dist <= self.userRange then
										noFired = noFired + 1;
							...
							if noFired >= self.capacity then
								...
								local shot = CreateAEmitter("D37-S Pistol Unload Sound Emitter");
								shot.Pos = self.Pos;
								MovableMan:AddParticle(shot);
								
								reload = CreateMOPixel("D37-S Pistol Reload");
								reload.Pos = user.Pos;
								...
								local shot = CreateAEmitter("D37-S Pistol Shot Sound Emitter");
								shot.Pos.X = self.Pos.X + math.cos(aimAngle) * self.muzzleOffset;
								shot.Pos.Y = self.Pos.Y + math.sin(aimAngle) * self.muzzleOffset * -1;
								MovableMan:AddParticle(shot);
								
								fired = CreateMOPixel("D37-S Pistol Fired");
								fired.Pos = user.Pos;
								MovableMan:AddParticle(fired);
								
								fired = CreateMOPixel("D37-S Pistol Refire");
								fired.Pos = user.Pos;
								MovableMan:AddParticle(fired);
							end
						end
