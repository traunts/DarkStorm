...
function Update(self)
	...
	for actor in MovableMan.Actors do
		if
			...
			and actor.PresetName ~= "D2-F Tengu Bushi"
			and actor.PresetName ~= "D4-F Oni Bushi"
			...
			if SceneMan:CastFindMORay(self.Pos, distVect, actor.ID, findPos, 0, false, 5) then
				...
				if dist <= radius then
					...
					for particle in MovableMan.Particles do
						if
							actor.Mass <= self.maxMass
							and particle.PresetName == "Ignited Fire Emitter"
							and flameFound == false
						then
