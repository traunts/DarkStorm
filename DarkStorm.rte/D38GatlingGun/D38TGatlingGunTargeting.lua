function D38TGatlingGunTargeting(self, ownCont, equip, target)
	local sensorRange = 600;
	...
	if not(MovableMan:ValidMO(curTarget)) then
		...
		for actor in MovableMan.Actors do
			...
			if actor.Team ~= self.Team and not(actor:IsDead()) then
				...
				local found = SceneMan:CastFindMORay(self.Pos, distVect, actor.ID, foundPoint, self.Team, 128, false, 0);
