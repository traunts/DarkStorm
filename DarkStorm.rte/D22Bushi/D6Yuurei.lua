...
function Update(self)
	if not (self:IsDead()) then
		...
		for i = 0, MovableMan:GetMOIDCount() do
			if MovableMan:GetRootMOID(i) == self.ID then
				...
				if
					...
					or (
						object:IsDevice()
						and (object.PresetName == "D35-S Assault Rifle" or object.PresetName == "D37-S Pistol")
					)
				then
					if self.activeCamo then
						object.Scale = 0
		...
		if self.phase == 0 then
			if
				UInputMan:KeyPressed(meleeKeyNum)
			...
			then
				...
				if self:GetAltitude(0, 0) > self.minClingAltitude then
					...
					local checkVect = Vector(
						math.cos(self:GetAimAngle(true)) * self.slashDist * 2,
						-math.sin(self:GetAimAngle(true)) * self.slashDist * 2
					)
					local checkPos = self.Pos
						+ Vector(
							math.cos(self:GetAimAngle(true)) * self.slashStartDistZero,
							-math.sin(self:GetAimAngle(true)) * self.slashStartDistZero
						)
					...
					if targetID ~= 255 then
						...
						doSlash = true
					elseif
						SceneMan:CastObstacleRay(checkPos, checkVect, strikePoint, lastPoint, self.ID, self.Team, -1, 0)
						< 0
					then
						--no terrain found
						doSlash = true
				...
				if doSlash then
					...
					local offsetVect = Vector(
						math.cos(diffAngle) * self.clingOffset,
						-math.sin(diffAngle) * self.clingOffset
					)
		...
		elseif self.phase == 1 then
			if MovableMan:ValidMO(self.slash) then
				...
				if self.slashAnimTimer:IsPastSimMS(self.slashAnimTime) then
					...
					local hit = false
					local checkVect = Vector(
						math.cos(self.slash.RotAngle) * self.slashDist,
						-math.sin(self.slash.RotAngle) * self.slashDist
					)
					local checkPos = self.Pos
						+ Vector(
							math.cos(self.slash.RotAngle) * slashStartDist,
							-math.sin(self.slash.RotAngle) * slashStartDist
						)
					local targetID = SceneMan:CastMORay(checkPos, checkVect, self.ID, self.Team, 0, false, 0)
					...
					if hit then
						...
						stab.Vel = self.Vel
							+ Vector(
								math.cos(self.slash.RotAngle) * self.slashSpeed,
								-math.sin(self.slash.RotAngle) * self.slashSpeed
							)
		...
		elseif self.phase == 2 then
			if
				(UInputMan:KeyPressed(meleeKeyNum) and ownCont.Player ~= -1)
				or ownCont:IsState(Controller.BODY_JUMPSTART)
				or ownCont:IsState(Controller.WEAPON_RELOAD)
			then
				self.phase = 0
			else
				self.Vel = Vector(0, -self.gravAcc)
