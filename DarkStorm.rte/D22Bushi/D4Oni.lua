...
function Update(self)
	if not(self:IsDead()) then
		...
		if self.packTimer:IsPastSimMS(self.packTime) then
			...
			for i = 0, MovableMan:GetMOIDCount() do
				if MovableMan:GetRootMOID(i) == self.ID then
					...
					if (object.PresetName == "D38GatlingGun Pack" or object.PresetName == "D107RevolverLaser Pack" or object.PresetName == "D52GrenadeMachineGun HE Pack" or object.PresetName == "D52GrenadeMachineGun EMP Pack" or object.PresetName == "D52GrenadeMachineGun Incendiary Pack") and object.Scale == 1 then
						noPack = false;
						break;
					end
				end
			...
			if noPack then
				...
				for device in MovableMan.Items do
					if device.PresetName == "D38 Gatling Gun 6mm Caseless Supply" or device.PresetName == "D107 Revolver Laser 50kW FCG Supply" or device.PresetName == "D52 Grenade Machine Gun HE Grenade Supply" or device.PresetName == "D52 Grenade Machine Gun EMP Grenade Supply" or device.PresetName == "D52 Grenade Machine Gun Incendiary Grenade Supply" then
						...
						if dist <= self.packRange then
							...
							elseif device.PresetName == "D52 Grenade Machine Gun HE Grenade Supply" then
								for i = 0, MovableMan:GetMOIDCount() do
									if MovableMan:GetRootMOID(i) == self.ID then
										object = MovableMan:GetMOFromID(i);
										if object.PresetName == "D52GrenadeMachineGun HE Pack" and object.Scale == 0 then
											object.Scale = 1;
											object.Mass = self.packMass;
											device.Lifetime = 1;
							...
							elseif device.PresetName == "D52 Grenade Machine Gun EMP Grenade Supply" then
								for i = 0, MovableMan:GetMOIDCount() do
									if MovableMan:GetRootMOID(i) == self.ID then
										object = MovableMan:GetMOFromID(i);
										if object.PresetName == "D52GrenadeMachineGun EMP Pack" and object.Scale == 0 then
											object.Scale = 1;
											object.Mass = self.packMass;
											device.Lifetime = 1;
							...
							elseif device.PresetName == "D52 Grenade Machine Gun Incendiary Grenade Supply" then
								for i = 0, MovableMan:GetMOIDCount() do
									if MovableMan:GetRootMOID(i) == self.ID then
										object = MovableMan:GetMOFromID(i);
										if object.PresetName == "D52GrenadeMachineGun Incendiary Pack" and object.Scale == 0 then
											object.Scale = 1;
											object.Mass = self.packMass;
											device.Lifetime = 1;
		...
		if self.phase == 0 then
			...
			if UInputMan:KeyHeld(meleeKeyNum) and ownCont.Player ~= -1 and not(ownCont:IsState(Controller.WEAPON_RELOAD)) then
				...
				self.chargeMeter.Frame = ((self.punchForce - self.minForce)/ (self.maxForce - self.minForce)) * (self.chargeMeter.FrameCount - 1);
		...
		elseif self.phase == 1 then
			...
			if UInputMan:KeyHeld(meleeKeyNum) and ownCont.Player ~= -1 and not(ownCont:IsState(Controller.WEAPON_RELOAD)) then
				...
				if MovableMan:ValidMO(self.grappleTarget) then
					...
					self.cooldownMeter.Frame = ((self.punchForce - self.minForce)/ (self.maxForce - self.minForce)) * (self.cooldownMeter.FrameCount - 1);
					...
					if self.doubleTapSide and not(self.doubleTapTimer:IsPastSimMS(self.doubleTapTime)) then
						...
						local targetID = SceneMan:CastMORay(self.Pos + self.normalOffset, checkVect, self.ID, self.Team, -1, false, 0);
				...
				if self.checkPunch then
					...
					if targetID ~= 255 and target.GetsHitByMOs then
						...
						if SceneMan:CastFindMORay(checkPos, checkVect, targetBaseID, strikePoint, -1, false, 0) then
							...
							local strikeVel = ((self.punchForce - self.minForce)/ (self.maxForce - self.minForce)) * self.strikeMaxVel;
							if self.punch.PresetName == "D4Punch Light" then
		...
		for i = 0, MovableMan:GetMOIDCount() do
			if MovableMan:GetRootMOID(i) == self.ID then
				object = MovableMan:GetMOFromID(i);
				if (object.PresetName == "D38GatlingGun Pack" or object.PresetName == "D107RevolverLaser Pack" or object.PresetName == "D52GrenadeMachineGun Pack") and object.Scale == 1 then
					noPack = false;
					break;
				end
			end
