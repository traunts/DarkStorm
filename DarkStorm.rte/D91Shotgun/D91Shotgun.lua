function Create(self)
	...
	if MovableMan:ValidMO(user) and user:IsActor() then
		...
		if not(MovableMan:ValidMO(disruption)) then
			...
			local reload = nil;
			for particle in MovableMan.Particles do
				if particle.PresetName == "D91 Shotgun Reload" then
					local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
					local dist = distVect.Magnitude;
					if dist <= self.userRange then
						reload = particle;
			...
			if MovableMan:ValidMO(reload) then
				...
				if UInputMan:KeyPressed(secondaryFireKeyNum) then
					local empty = CreateAEmitter("D91 Shotgun Empty Sound Emitter");
					empty.Pos = self.Pos;
				...
				if reload.Mass > self.reloadTime then
					...
					local loading = CreateAEmitter("D91 Shotgun Load Sound Emitter");
					...
					for i = 0, MovableMan:GetMOIDCount() do
						if MovableMan:GetRootMOID(i) == user.ID then
							...
							if object.PresetName == "D91 Shotgun Indicator Green" then
								object.Scale = 1;
							elseif object.PresetName == "D91 Shotgun Indicator Yellow" or object.PresetName == "D91 Shotgun Indicator Red" or object.PresetName == "D91 Shotgun Indicator Blue" then
								object.Scale = 0;
							end
						end
					end
				...
				if userCont:IsPlayerControlled(-1) then
					...
					local refire = nil;
					for particle in MovableMan.Particles do
						if particle.PresetName == "D91 Shotgun Refire" then
							local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
							local dist = distVect.Magnitude;
							if dist <= self.userRange then
								refire = particle;
					...
					if MovableMan:ValidMO(refire) then
						...
						if UInputMan:KeyPressed(secondaryFireKeyNum) then
							...
							local noFired = 0;
							for particle in MovableMan.Particles do
								if particle.PresetName == "D91 Shotgun Fired" then
									local distVect = SceneMan:ShortestDistance(user.Pos, particle.Pos, true);
									local dist = distVect.Magnitude;
									if dist <= self.userRange then
										noFired = noFired + 1;
							...
							if noFired >= self.capacity then
								...
								local shot = CreateAEmitter("D91 Shotgun Unload Sound Emitter");
								shot.Pos = self.Pos;
								MovableMan:AddParticle(shot);
								
								reload = CreateMOPixel("D91 Shotgun Reload");
								reload.Pos = user.Pos;
								...
								for i = 0, MovableMan:GetMOIDCount() do
									if MovableMan:GetRootMOID(i) == user.ID then
										local object = MovableMan:GetMOFromID(i);
										if object.PresetName == "D91 Shotgun Indicator Blue" then
											object.Scale = 1;
										elseif object.PresetName == "D91 Shotgun Indicator Green" or object.PresetName == "D91 Shotgun Indicator Yellow" or object.PresetName == "D91 Shotgun Indicator Red" then
											object.Scale = 0;
										end
									end
								end
								...
								--fire a shot
								for i = 1, self.noShot do
									local shot = CreateMOPixel("000 Buckshot");
									...
									shot.Vel.X = math.cos(aimAngle + posSpread + negSpread) * self.fireVelocity;
									shot.Vel.Y = math.sin(aimAngle + posSpread + negSpread) * self.fireVelocity * -1;
									shot:SetWhichMOToNotHit(user, 1000);
									MovableMan:AddParticle(shot);
								end
								
								local flash = CreateMOSRotating("Muzzle Flash 12-Gauge Underslung Shotgun");
								...
								local shot = CreateAEmitter("D91 Shotgun Shot Sound Emitter");
								shot.Pos.X = self.Pos.X + math.cos(aimAngle) * self.muzzleOffset;
								shot.Pos.Y = self.Pos.Y + math.sin(aimAngle) * self.muzzleOffset * -1;
								MovableMan:AddParticle(shot);
								
								fired = CreateMOPixel("D91 Shotgun Fired");
								fired.Pos = user.Pos;
								MovableMan:AddParticle(fired);
								
								fired = CreateMOPixel("D91 Shotgun Refire");
								fired.Pos = user.Pos;
								MovableMan:AddParticle(fired);
								...
								if noFired + 1 >= self.capacity then
									for i = 0, MovableMan:GetMOIDCount() do
										if MovableMan:GetRootMOID(i) == user.ID then
											local object = MovableMan:GetMOFromID(i);
											if object.PresetName == "D91 Shotgun Indicator Red" then
												object.Scale = 1;
											elseif object.PresetName == "D91 Shotgun Indicator Green" or object.PresetName == "D91 Shotgun Indicator Yellow" or object.PresetName == "D91 Shotgun Indicator Blue" then
												object.Scale = 0;
											end
										end
									end
								elseif noFired + 1 >= self.capacity / 2 then
									for i = 0, MovableMan:GetMOIDCount() do
										if MovableMan:GetRootMOID(i) == user.ID then
											local object = MovableMan:GetMOFromID(i);
											if object.PresetName == "D91 Shotgun Indicator Yellow" then
												object.Scale = 1;
											elseif object.PresetName == "D91 Shotgun Indicator Green" or object.PresetName == "D91 Shotgun Indicator Red" or object.PresetName == "D91 Shotgun Indicator Blue" then
												object.Scale = 0;
											end
										end
									end
